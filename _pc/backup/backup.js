  ...
  window.addEventListener("scroll", (e) => {
    ...
    if (isFixed === true && swiperActive === false) {
      ...
      swiperInner.addEventListener("wheel", (e) => {
        ...
        if (isFixed === true && swiperActive === false && cateSwiper.activeIndex === 0 && e.deltaY < 0) {
          console.log("object");
          unsetFixed("scrollUp");
        }
...
function categorySlide3() {
  ...
  window.addEventListener("scroll", (e) => {
    ...
    // 스크롤이 카테고리 섹션을 넘어섰을때,
    if (cateSwiperSection.getBoundingClientRect().top <= 0) {
      // 첫번째 슬라이드, 마지막 슬라이드에서 마우스 휠 이벤트 활성화
      swiperInner.addEventListener("wheel", (e) => {
        // 첫번째 슬라이드에서 위로 스크롤 시, 고정 해제
        ...
        // 마지막 슬라이드에서 아래로 스크롤 시, 고정 해제
        if (cateSwiper.activeIndex === cateSwiper.slides.length - 1 && e.deltaY > 0) {
          DisableMouseWheel();
        }
      });

      // 첫번째 슬라이드에서 아래로 스크롤시, 고정 활성화
      if (currentSlideIndex === 0 && deltaY > 0) {
        ActiveMouseWheel();
      }
    }
    // 스크롤이 카테고리 섹션에 도달했을때, 첫번째 슬라이드가 아니고, 위로 스크롤 시, 고정 활성화
    if (cateSwiperSection.getBoundingClientRect().top >= 0 && cateSwiper.activeIndex !== 0 && deltaY < 0) {
      ActiveMouseWheel();
    }
